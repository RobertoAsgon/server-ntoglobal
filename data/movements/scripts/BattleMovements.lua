...
addOutfitCondition(conditionBlack, {lookType = 613, lookHead = 114, lookBody = 114, lookLegs = 114, lookFeet = 114})
local conditionRed = createConditionObject(CONDITION_OUTFIT)
setConditionParam(conditionRed, CONDITION_PARAM_TICKS, -1)
addOutfitCondition(conditionRed, {lookType = 546, lookHead = 94, lookBody = 94, lookLegs = 94, lookFeet = 94})
...
doPlayerSendTextMessage(cid, MESSAGE_INFO_DESCR, "Voce entrou para o time " .. getMyTeam[3] .. "!")
setGlobalStorageValue(_Lib_Battle_Info.storage_count, getGlobalStorageValue(_Lib_Battle_Info.storage_count)-1)
end
if getGlobalStorageValue(_Lib_Battle_Info.storage_count) == 0 then
removeBattleTp()
doBroadcastMessage("Prepare-se a batalha começará em dois minutos.")
addEvent(doBroadcastMessage, 2*60*1000-500, "Guerra!!")
